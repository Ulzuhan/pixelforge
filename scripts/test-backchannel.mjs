...
const PUERTO_IDP = Number(process.env.PUERTO_IDP ?? 9994);
const ORIGEN_IDP = `http://127.0.0.1:${PUERTO_IDP}`;
const EMISOR = `${ORIGEN_IDP}/application/o/pixelforge`;
const CLIENT_ID = process.env.CLIENT_ID ?? "pixelforge-pruebas";
...
// PixelForge no tiene listado ni "quién soy": la puerta con sesión es
// /api/removebg, que responde 401 sin ella. Se manda un cuerpo vacío a
// propósito — lo que se mira es si la deja pasar, no que procese nada, y así
// la prueba no depende de tener los modelos instalados.
const rechazada = async () =>
  (await fetch(`${BASE}/api/removebg`, { method: "POST", headers: { cookie: cabecera() } })).status === 401;

check("y la sesión sirve", await rechazada(), false);

const cierre = await avisar(firmar({ carga: { sub: SUB_REAL } }));
check("el aviso se atiende", cierre.status, 200);

check("la sesión YA NO sirve: eso es la revocación llegando", await rechazada(), true);
...
check("y quien no fue revocado sigue fuera igualmente (ya lo estaba)", await rechazada(), true);
