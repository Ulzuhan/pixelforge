...
export function oidcConfig(): OidcConfig | null {
  ...
  const issuer = validUrl(process.env.PIXELFORGE_OIDC_ISSUER?.trim(), true);
  const internalOrigin = validUrl(
    process.env.PIXELFORGE_OIDC_INTERNAL_BASE?.trim() || (issuer ? new URL(issuer).origin : undefined),
    false
  );
  const redirectUri = validUrl(process.env.PIXELFORGE_OIDC_REDIRECT_URI?.trim(), true);
...
// Lo que se espera al proveedor antes de rendirse. Estaba escrito como 10_000
// en cada llamada; con el discovery pasaban a ser tres sitios, y tres copias de
// un número acaban divergiendo.
 ...
 * Nada de eso lo lleva esta aplicación, y sin esta variable no se enlaza a ninguna parte.
 */
export function accountUrl(): string | null {
  return enlaceExterno(process.env.PIXELFORGE_ACCOUNT_URL);
}

export function enrollUrl(): string | null {
  return enlaceExterno(process.env.PIXELFORGE_ENROLL_URL);
}
