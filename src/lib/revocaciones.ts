 ...
 * Dónde vive la lista.
 *
 * PixelForge no guarda nada de nadie —esa es su gracia— así que no tiene un
 * almacén donde poner esto. Lo único suyo que sobrevive a un reinicio es el
 * directorio de los modelos, y ahí va, en su propio subdirectorio: la lista
 * TIENE que sobrevivir a un reinicio, porque si no, revocar a alguien duraría
 * hasta el siguiente despliegue.
 *
 * Lo que se guarda sigue siendo un identificador opaco y una fecha, que caduca
 * sola. Nada que se pueda leer como «datos de una persona».
 */
function archivo(): string {
  const raiz = process.env.PIXELFORGE_STATE_DIR?.trim()
    || join(process.env.U2NET_HOME?.trim() || "/models", "estado");
  return join(raiz, "revocaciones.json");
}
